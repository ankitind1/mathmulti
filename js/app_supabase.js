...
function questionAt(index, seed, difficulty) {
...
async function joinChannel(code) {
  ...
  room.channel.on('broadcast', { event: 'ended' }, ()=>{
    room.status = 'ended'; showResults();
  });
...
function renderQuestion() {
  const q = questionAt(room.index, room.seed, room.difficulty);
  $('questionBox').textContent = `${q.a} + ${q.b} = ?`;
  $('answerInput').value = '';
}
...
function showResults() {
  ...
  const arr = Array.from(room.players.values())
    .map(p=>({ name: p.name, score: p.score||0, eliminated: !!p.eliminated }))
